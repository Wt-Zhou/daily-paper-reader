...
tags: ["query:ad-ei"]
score: 8.0
evidence: 用于视觉-语言导航的动态几何基础模型
tldr: 针对现有视觉语言导航（VLN）在动态现实场景中泛化能力不足的问题，本文提出了DyGeoVLN框架。该框架通过跨分支特征融合将动态几何基础模型引入VLN，实现了显式的3D空间表示和视觉语义推理。此外，引入了一种无位姿且自适应分辨率的令牌剪枝策略，有效减少了长程导航中的计算开销。实验证明，该方法在多个基准测试中达到了SOTA性能，并展现出极强的现实环境鲁棒性。
selection_source: fresh_fetch
motivation: 现有的VLN方法大多依赖静态场景假设，难以应对动态且复杂的现实世界导航挑战。
method: 提出DyGeoVLN框架，结合动态几何基础模型进行特征融合，并采用自适应令牌剪枝策略优化长程推理效率。
result: 在多个主流VLN基准测试上取得了最先进的性能，并在真实物理环境中表现出显著的鲁棒性。
conclusion: 通过融合动态几何感知与高效的令牌管理，DyGeoVLN显著提升了智能体在动态未知环境中的导航能力。
---
