...
tags: ["query:ad-ei"]
score: 7.0
evidence: 物理AI智能体与机器人的主动推理
tldr: 本文从工程视角探讨了主动推理（AIF）在物理AI智能体中的应用。针对机器人等受限系统在复杂环境中的局限性，文章基于自由能原理，提出将感知、学习、规划与控制统一在最小化变分自由能的框架下。通过在因子图上实现反应式消息传递，该方法支持局部并行计算，能有效应对实时性、异步数据及功耗波动等挑战，为构建具备鲁棒性和可扩展性的物理智能体提供了理论与架构支撑。
selection_source: fresh_fetch
motivation: 物理AI智能体在资源受限的开放环境中表现远逊于生物系统，亟需一种统一且高效的计算框架来提升其适应能力。
method: 基于自由能原理，利用因子图上的反应式消息传递来实现变分自由能的最小化，从而在单一计算目标下统一各项认知功能。
result: 该架构支持事件驱动和局部自适应计算，使智能体在资源减少时能优雅降级，并能通过耦合形成跨尺度的同构架构。
conclusion: 主动推理为物理AI提供了一个原则性的工程基础，能够有效解决实时物理操作中的资源约束与环境适应性问题。
---

## 摘要
物理人工智能体（如机器人和其他在严格且波动的资源约束下运行的具身系统）在开放式现实环境中的能力仍远逊于生物体。本文认为，基于自由能原理（Free Energy Principle, FEP）的主动推理（Active Inference, AIF）为缩小这一差距提供了原则性基础。我们从第一性原理出发阐述这一论点，遵循从概率论到贝叶斯机器学习、变分推理，再到主动推理和反应式消息传递的逻辑链条。从 FEP 的视角来看，在适当的假设下，随时间维持其结构和功能完整性的系统可以被描述为在最小化变分自由能（VFE），而 AIF 通过将感知、学习、规划和控制统一在单一计算目标内，使这一过程具有可操作性。我们展示了 VFE 最小化可以通过因子图上的反应式消息传递自然实现，其中推理源自局部并行计算。这种实现方式与物理运行的约束高度契合，包括硬截止时间、异步数据、波动的功率预算以及不断变化的环境。由于反应式消息传递是事件驱动、可中断且具有局部适应性的，因此在资源减少时性能会平稳下降，同时模型结构可以进行在线调整。我们进一步表明，在适当的耦合和粗粒化条件下，耦合的 AIF 智能体可以被描述为更高层级的 AIF 智能体，从而产生一种跨尺度基于相同消息传递原语的同构架构。我们的贡献不在于实证基准测试，而是为工程界提供了一个清晰的理论和架构案例。
