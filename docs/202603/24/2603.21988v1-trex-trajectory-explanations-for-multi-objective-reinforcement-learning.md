...
tags: ["query:ad-ei"]
score: 6.0
evidence: 用于决策的多目标强化学习
tldr: 针对多目标强化学习（MORL）中决策过程不透明的问题，本文提出了TREX框架。该框架利用轨迹归因技术，将专家策略生成的轨迹聚类为语义段，并通过训练排除特定聚类的互补策略来量化不同行为对帕累托权衡的影响。在MuJoCo多目标环境下的实验证明，TREX能有效识别并量化影响目标权衡的关键行为模式，显著提升了MORL模型的可解释性。
selection_source: fresh_fetch
motivation: 现有的强化学习解释方法主要针对单目标奖励，无法有效解释多目标场景下复杂的决策权衡和用户偏好。
method: 提出基于轨迹归因的TREX框架，通过聚类专家轨迹段并对比缺失特定行为后的策略偏差来量化行为对目标的贡献。
result: 在HalfCheetah、Ant和Swimmer等MuJoCo多目标实验中，该框架成功分离并量化了影响策略权衡的具体行为特征。
conclusion: TREX为多目标强化学习提供了一种新颖的轨迹级解释方案，增强了对智能体在冲突目标间决策逻辑的理解。
---
