...
tags: ["query:ad-ei"]
score: 8.0
evidence: 基于VLM的机器人操作优化奖励生成
tldr: 针对机器人强化学习中奖励函数设计难的问题，本文提出了一种基于视觉语言模型（VLM）的在线奖励生成框架。通过在多源大规模数据集上训练，该模型能根据视觉观测实时生成包含过程、完成度和时间对比的多维奖励信号。实验表明，该方法能以零样本方式在30次迭代内显著提升模仿学习策略的成功率，实现了高效的在线策略优化，消除了对人工奖励工程的依赖。
selection_source: fresh_fetch
motivation: 强化学习在机器人领域应用受限，主要瓶颈在于难以设计出能够适应复杂环境且具备泛化能力的奖励函数。
method: 将预训练的VLM转化为在线奖励生成器，通过多源数据训练使其能基于视觉观测提供过程、完成及时间对比等多维度反馈。
result: 在具有挑战性的长程操作任务中，该方法在30次迭代内显著提升了初始模仿学习策略的表现，且展现出强大的零样本泛化性能。
conclusion: VLM生成的奖励信号可以作为可靠的反馈机制，有效解决执行错误并实现高效的机器人在线策略精炼。
---

## 摘要
强化学习 (RL) 在优化机器人操纵策略方面展现出巨大潜力，但其有效性仍受限于设计可泛化奖励函数的难度。在本文中，我们提出了一个在线策略优化框架，通过将基础视觉语言模型 (VLM) 适配为在线奖励生成器。我们基于最先进的 VLM 开发了一个鲁棒且可扩展的奖励模型，该模型在包含真实机器人轨迹、人机交互以及多样化模拟环境的大规模多源数据集上进行了训练。与以往事后评估整个轨迹的方法不同，我们的方法利用 VLM 根据当前的视觉观测制定多维奖励信号，包括过程奖励、完成奖励和时间对比奖励。我们以通过模仿学习 (IL) 训练的基础策略为初始，利用这些 VLM 奖励引导模型以闭环方式纠正次优行为。我们在具有挑战性的长程操纵基准测试中评估了我们的框架，这些基准需要顺序执行和精确控制。至关重要的是，我们的奖励模型在这些测试环境中以纯零样本 (zero-shot) 方式运行。实验结果表明，我们的方法在仅 30 次 RL 迭代内就显著提高了初始 IL 策略的成功率，展示了卓越的样本效率。这一实证证据表明，VLM 生成的信号可以提供可靠的反馈来解决执行错误，从而有效地消除了对手工奖励工程的需求，并促进了机器人学习的高效在线优化。
