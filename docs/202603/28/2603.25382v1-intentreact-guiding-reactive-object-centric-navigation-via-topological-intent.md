...
tldr: 针对目标驱动视觉导航中全局拓扑规划与局部反应式控制脱节的问题，本文提出 IntentReact 框架。该框架引入“意图”作为紧凑接口，将全局拓扑引导编码为低维方向信号，指导局部策略在局部观测误导时及时调整方向。实验证明，该方法在保持反应式控制鲁棒性的同时，显著提升了导航成功率和执行质量。
selection_source: fresh_fetch
motivation: 现有的拓扑导航方法在局部决策时仅依赖当前视野，容易因缺乏全局信息而导致机器人偏离目标方向或陷入无效路径。
method: 提出一种意图条件化的导航框架，利用全局拓扑引导生成的低维方向信号来偏置局部航点预测，确保运动与全局目标一致。
result: 实验结果表明，IntentReact 在导航成功率和执行质量上均显著优于此前基于对象中心的导航方法。
conclusion: 通过引入拓扑意图作为全局规划与局部控制的桥梁，能有效克服局部感知的局限性，提升机器人在复杂环境下的导航效率。
---
