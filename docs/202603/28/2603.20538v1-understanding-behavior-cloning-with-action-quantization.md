...
tldr: 行为克隆（BC）在机器人和自动驾驶中至关重要，自回归模型常通过动作量化处理连续控制，但其理论基础尚不明确。本文为动作量化下的BC提供了理论框架，分析了量化误差的传播与统计样本复杂度的关系。研究证明，在动力学稳定且策略平滑的条件下，量化动作的BC能达到最优样本复杂度，并提出了一种模型增强方法以优化误差界限，揭示了量化误差与统计复杂度的内在联系。
selection_source: fresh_fetch
motivation: 尽管动作量化在自回归行为克隆模型中被广泛应用，但其误差传播机制和统计样本复杂度的理论基础仍不清晰。
method: 通过数学分析量化误差在时间步上的传播，并提出一种无需策略平滑性假设的模型增强方法来优化误差界限。
result: 证明了采用对数损失的量化动作BC能实现最优样本复杂度，且在特定条件下量化误差对时间步仅呈多项式级依赖。
conclusion: 本文建立了动作量化行为克隆的理论基础，明确了量化方案的适用条件，并揭示了量化误差与统计复杂度的基本限制。
---
