...
tldr: 本研究针对3D物体功能区域定位中的开放词汇泛化及细粒度几何对齐难题，提出了一种两阶段跨模态框架。该框架利用大语言模型生成部件感知指令以增强语义，并通过原型聚合（APA）和物体内关系建模（IORM）提升几何一致性与区分度。实验证明，该方法在多个基准测试中均优于现有技术，显著提升了具身智能对3D环境的理解能力。
selection_source: fresh_fetch
motivation: 现有的3D功能区域定位方法在处理开放词汇、细粒度几何对齐及部件级语义一致性方面仍存在显著局限。
method: 提出一种结合LLM生成部件感知指令、原型聚合（APA）及物体内关系建模（IORM）的两阶段跨模态对齐框架。
result: 在多个基准测试上的实验结果表明，该方法在性能上显著超越了当前主流方法。
conclusion: 通过结合语义增强与几何原型对齐，该研究有效解决了开放词汇环境下3D功能区域的精准定位问题。
---
