...
tags: ["query:ad-ei"]
score: 7.0
evidence: 用于自主机器人视觉里程计的强化学习
tldr: 针对资源受限机器人视觉里程计（VO）前端参数难以适应多变环境的问题，本文提出首个基于图像条件的强化学习框架。该框架通过轻量级CNN编码器感知图像纹理，动态调整特征检测与跟踪参数。实验表明，该方法在显著降低计算成本的同时，大幅提升了特征跟踪长度和系统鲁棒性，且具有良好的仿真到现实的泛化能力。
selection_source: fresh_fetch
motivation: 传统的视觉里程计前端依赖固定的手工调优参数，难以应对光照变化、运动模糊和纹理密度波动等复杂现实场景。
method: 将前端参数配置建模为序列决策问题，利用强化学习策略根据图像视觉特征实时预测并调整特征检测与跟踪参数。
result: 在TartanAirV2和TUM RGB-D数据集上，该方法实现了3倍长的特征跟踪距离和3倍低的计算成本。
conclusion: 通过图像感知的在线参数自适应调整，可以有效提升视觉里程计在动态环境下的鲁棒性与运行效率。
---

## 摘要
资源受限的自主机器人依赖稀疏直接法和半直接法视觉（惯性）里程计（VO）流水线，因为它们在精度、鲁棒性和计算成本之间提供了良好的权衡。然而，大多数系统的性能关键取决于控制特征检测、跟踪和离群点剔除的手调超参数。这些参数在部署期间通常是固定的，尽管其最优值随场景特征（如纹理密度、光照、运动模糊和传感器噪声）而变化，从而导致在现实环境中的性能脆弱。我们提出了首个用于在线调优 VO 前端参数的图像条件强化学习框架，有效地将专家经验嵌入到系统中。我们的核心思想是将前端配置建模为一个序列决策问题，并学习一种直接将视觉输入映射到特征检测和跟踪参数的策略。该策略在训练期间使用轻量级纹理感知 CNN 编码器和特权评论家（privileged critic）。与以往仅依赖内部 VO 统计数据的基于强化学习的方法不同，我们的方法观察图像内容，并在跟踪性能下降之前主动调整参数。在 TartanAirV2 和 TUM RGB-D 上的实验表明，尽管完全在仿真中训练，特征跟踪长度增加了 3 倍，计算成本降低了 3 倍。
