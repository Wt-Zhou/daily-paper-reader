...
## 摘要
协作任务排序与多智能体路径规划（CTS-MAPF）问题要求智能体在避免碰撞的同时完成一系列任务，由于其组合复杂性，这带来了巨大的挑战。本研究提出了 CTS-PLL，这是一个分层框架，它通过两项关键改进扩展了基于配置的 CTS-MAPF 规划范式：一是利用完备规划方法进行局部重规划的锁定智能体检测与释放机制，二是基于大邻域搜索（LNS）的随时优化程序。这些改进确保了在密集环境中的鲁棒性，并能够持续提升解的质量。在稀疏和密集基准测试上的广泛评估表明，与现有方法相比，CTS-PLL 在保持竞争力的运行效率的同时，实现了更高的成功率和解质量。真实世界的机器人实验进一步证明了该方法在实践中的可行性。
