...
tldr: 本研究针对强化学习内部学习行为难以解释的问题，提出了一种多维度的损失函数景观可视化框架。该框架通过3D评论家损失曲面、演员损失景观、更新轨迹及状态-TD映射，深入剖析了价值估计、策略优化与TD信号的交互机制。以航天器姿态控制中的ADHDP算法为例，揭示了不同训练稳定器对学习稳定性的影响，为RL算法设计提供了系统性的解释工具。
selection_source: fresh_fetch
motivation: 强化学习在动态控制系统中应用广泛，但其内部学习动态及各组件间的交互机制缺乏直观的解释手段。
method: 构建了一个包含评论家/演员损失景观、权重更新轨迹和状态-TD映射的综合可视化框架。
result: 通过航天器控制案例，展示了该框架如何揭示训练稳定器和目标更新对优化几何结构及学习稳定性的改变。
conclusion: 该框架为分析和比较不同强化学习算法的设计提供了系统且具有可解释性的分析工具。
---

## 摘要
强化学习算法已广泛应用于动力学和控制系统。然而，解释其内部学习行为仍然是一个挑战。在作者之前的研究中，提出了一种评价网络匹配损失景观可视化方法来研究评价网络的训练。本研究将该方法扩展为一个框架，提供了学习动力学的多视角视图，阐明了价值估计、策略优化和时序差分（TD）信号在训练过程中是如何相互作用的。所提出的框架包含四个互补的组成部分：评价网络匹配损失曲面的三维重建，展示了 TD 目标如何塑造优化几何结构；冻结评价网络下的执行网络损失景观，揭示了策略如何利用该几何结构；结合时间、贝尔曼误差和策略权重的轨迹，指示了更新如何在曲面上移动；以及识别驱动这些更新的状态区域的状态-TD 映射。本研究以用于航天器姿态控制的动作依赖启发式动态规划（ADHDP）算法作为案例研究。该框架被应用于比较几种 ADHDP 变体，并展示了训练稳定器和目标更新如何改变优化景观并影响学习稳定性。因此，所提出的框架为分析不同算法设计的强化学习行为提供了一个系统且可解释的工具。
