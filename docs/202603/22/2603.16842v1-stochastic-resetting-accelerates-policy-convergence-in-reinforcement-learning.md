...
method: 在表格化网格环境和基于神经网络的连续控制任务中，引入间歇性将智能体返回固定参考状态的随机重置机制进行实验。
result: 随机重置通过截断无信息轨迹并增强价值传播，在不减少搜索时间的情况下依然能加速策略收敛，并显著提升深度强化学习在复杂任务中的表现。
conclusion: 随机重置是一种能够保持最优策略且易于调节的加速学习机制，成功将统计力学的经典现象转化为强化学习的优化原则。
---
