...
tldr: AsgardBench是一个旨在评估视觉驱动交互式规划能力的基准测试。与以往侧重导航或底层控制的基准不同，它专注于高层动作序列的生成与调整。通过提供108个具有状态变化的复杂任务，该基准要求模型仅凭图像、历史记录和简单的成功/失败信号进行实时规划修正。研究发现，领先的视觉语言模型在缺乏视觉输入或面对复杂状态追踪时表现大幅下降，揭示了其在视觉接地和动态规划方面的局限性。
selection_source: fresh_fetch
motivation: 现有的具身智能基准往往将推理与导航混淆，或提供过多的纠错反馈，无法有效隔离评估模型在最小反馈下的交互式规划能力。
method: 开发了包含12类任务、108个实例的模拟器基准，通过改变物体状态和场景配置来强制模型进行条件分支判断和执行中的计划修复。
result: 评估显示，领先的视觉语言模型在没有视觉输入时性能骤降，暴露出其在视觉接地和状态追踪方面的显著弱点。
conclusion: AsgardBench 证明了当前模型在利用视觉观察来适应意外情况并修正计划方面仍存在巨大挑战。
---
